...
* **Despliegue:** (Requiere la base de datos central iniciada primero)
  ...
  ```
  *Para probar el cliente interactivo localmente, puedes usar el shortcut en la raíz `.\run_client.bat`.*
...
* **Despliegue:** Se construye y ejecuta mediante comandos de Docker puros (expone el puerto `8080`):
  ```bash
  cd area-manager
  docker build -t area-manager .
  docker run -d -p 8080:8080 --name area-manager area-manager
  ```
...
Para detener y eliminar el servicio de Go puro:
```bash
docker stop area-manager && docker rm area-manager
```
